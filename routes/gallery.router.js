...
const { server, db } = require('../db/firebase.js')

const base = require('../nueva.json')

const Gallery = require("../services/gallery.service.js");
const managment = new Gallery()
...
router.patch('/:collection/:id',configureUpload, async (req, res, next) => {
  ...
  uploadMiddleware(req, res, async (err) =>{
    if (err) {
      console.log('[ERROR EN PATCH]',err)
      return next(Boom.badRequest(err.message));
    }
